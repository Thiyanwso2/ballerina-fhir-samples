    ...
    *PatientSourceConnect;
    ...
    isolated function search(map<r4:RequestSearchParameter[]> searchParameters, r4:FHIRContext ctx) returns r4:Bundle|Patient[]|r4:FHIRError {
        ...
        if ctx.getRequestSearchParameters().hasKey("_id") {
            return searchPatientById(patientsArray, ctx);
        }
...
public isolated function searchPatientById(r4:Patient[] patients, r4:FHIRContext ctx) returns r4:Patient[]{

    r4:Patient[] response = [];
    
    string? id = getTheId(ctx);    
    if id is string { 
        response = from r4:Patient p in patients
                    where p.id == id
                    select p;
    } 
    return response; 
